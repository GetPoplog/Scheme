...
;;; This now uses the more efficient memo_T_2_n procedure, which requires
;;; an exact (==) match.
...
section PARSE_GEN memo_T_1_n Env => call_parse_gen  call_parse_gen_Env;
...
This procedure updates the  non-local  variables
   ...
   toks_wanted     A token that, if it had been  present, would give rise
                  to a legal parse
...
*/
vars n_max = 6, n_max1 = n_max+1;

define add_tok(tok,toks,L);
    if L==L_max then
        if member(tok,toks) then toks
        else
            lvars n = length(toks);
            if n==n_max then toks <> ['others'] ->> toks;
            elseif n==n_max1 then tok -> hd(toks); toks;
            else tok::toks
            endif;
        endif
    else
...
define record_HWM(L,tok,n);
  lvars L,tok,n;
    ...
    then                               ;;; Yes - so must be HWM.
        ;;;L -> L_max;
        add_tok(tok,toks_wanted,L) -> toks_wanted;
        caller(n) -> NT_tried;
...
define call_parse_gen(file,P_tok,Parse) -> L_out -> T;
  ...
  dlocal
       toks_wanted = [];                 ;;; Initialise toks_wanted
  ...
  unless L_out=[]  then                   ;;; Have we read all the tokens.
    ...
    and ispair(L.back.back) do            ;;; go through list until we find
      L.back -> L;                        ;;; the unexpanded section.
      i - 1 -> i;
    ...
    endwhile;
     ...
     lvars
       tok_found = if null(L) or null(tl(L)) then 'end of input'
                      else hd(L);
                      endif,
        L_here =
    ...
    mishap('Parse failed:\n looking for  \n  ' >< TW(toks_wanted)
            >< '\nfound "' >< tok_found
            >< '" parsing '
            >< if length(toks_wanted)>1 then '(among others) '
               else '' endif
            >< DP(NT_tried.pdprops)
            >< '\n as part of ' >< DP(NT_tried1.pdprops)
            >< '\n context of error..\n   ' ><
            Str_toks(L_here),
           []);
  endunless;
...
define call_parse_gen_Env(/*...*/ Env1);
  dlocal Env = Env1;
  chain(call_parse_gen);
enddefine;

define Str_toks(L);
  if null(L) then '\n'
  else L.hd><' '><Str_toks(L.tl)
  endif;
enddefine;


define DP(p);
  if p = "check" or p = "check_as_class" then
    'the singleton grammar <' ><tok_check >< ">"
  else "<" >< p >< ">"
  endif
enddefine;
...
define TW(toks_wanted)->str;
  lvars tsw,tok_wanted;
  ' ' -> str;
  for tsw on toks_wanted do
    hd(tsw) -> tok_wanted;
    str ><
    if isprocedure (tok_wanted) then
      'something satisfying ' >< pdprops(tok_wanted)
    elseif tok_wanted = newline then 'the new-line character'
    elseif isword(tok_wanted) then """ >< tok_wanted >< """
    else tok_wanted
    endif

      ><     if null(tl(tsw)) then ' ' else ' or ' endif
...
define recog(L,is_it) -> L1 -> Obj;
    ...
    if null(L) then                          ;;; If we have a null list, then
        add_tok(is_it,toks_wanted,L)-> toks_wanted;      ;;; record failure.
;;;        false -> L_max;
      return(false -> L1);
    endif;
...
vars procedure(
  ...
  new_line            = check_as_class(%newline%),
);



define identifier_g(L,LW_reserved) -> L1 -> Obj;
    lvars L,L1,Obj;
    if null(L) then
      add_tok('an identifier',toks_wanted,L) -> toks_wanted;
      return(false -> L1)
    endif;
    dest(L) -> L1 -> Obj;
    unless isword(Obj)
...
Parse an expression  using operator precedence.  L is the  list of  tokens,
Parse_aexpr is the  parser for recognising  atomic expressions, i.e.  those
that are combined together  with the operators, Parse_op  is the parser  to
recogise (binary) operators. Parse_unary is  the parser to recognise  unary
operators. Prec  is a  procedure  which returns  the precedence  of  binary
operators. Prec_unary is a procedure which returns the precedence of  unary
operators. is_more_binding(Op1,Op2) returns true if  Op1 is a more  binding
...
define procedures_dec94(LW_reserved);
    {%
        identifier_g(%LW_reserved%),
        id_any    ,
        obj_any   ,
        int       ,
        real      ,
